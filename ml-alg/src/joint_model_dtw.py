...
class FeatureToOutput(nn.Module):
    def __init__(self, joint_feature, positional_emb, transformer_encoder, linear, transform_seq_len, mode='fusion'):
        super().__init__()
    ...
    def forward(self, y, feature_sequence):
        ...
        pos_indices = torch.arange(self.transform_seq_len, device=y.device).unsqueeze(0).expand(batch_size, self.transform_seq_len)
